...
test.describe("Edit user profile", () => {
  ...
  test("Edit user profile details", async ({ page }) => {
    const updatedName = "Profile Details User"
    const discordHandle = "profile-user"

    await page.getByRole("button", { name: "Edit" }).click()
    await page.getByPlaceholder("Full name").fill(updatedName)
    await page.getByLabel("Discord handle").fill(discordHandle)
    await selectProfession(page, "Clinical researcher")
    await page.getByRole("button", { name: "Save" }).click()
    ...
    await expect(
      page.locator("form").getByText(discordHandle, { exact: true }),
    ).toBeVisible()
    await expect(
      page.locator("form").getByText("Clinical researcher", { exact: true }),
    ).toBeVisible()
  })
